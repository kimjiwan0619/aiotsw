...
// FND에 출력되는 문자 (0~9) 배열
const int FndFont[5] = { 0x6E, 0x79, 0x1C, 0x1C, 0x3F};
...
void FndSelect (int position) {
    ...
    for( i = 0; i < 6; i++ ) {
        if( i == position ) {
            digitalWrite( FndSelectPin[ i ], LOW ); // 선택된 FND의 Select 핀ON
        }
        else {
            digitalWrite( FndSelectPin[ i ], HIGH ); // 선택되지 않은 FND의Select 핀OFF
        }
    }
}
// FND를 출력하는 함수
...
int main() {
    int pos;
    int data[5] = { 0, 1, 2, 3, 4}; // 출력할 문자 데이터
    Init();
    while(1) {
        for( pos = 0; pos < 6; pos++ ) {
